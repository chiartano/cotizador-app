...
function prepareBaselines() {
  ...
  fs.writeFileSync(cSw, fs.readFileSync(cSw, 'utf8').replace("const CACHE_NAME = 'cotizador-v7.8';", "const CACHE_NAME = 'cotizador-v7.9-audit';"));
}
...
async function bridgeFlow(port, fromVersion, name) {
  ...
  await page.click('#pwa-update-now'); await waitUntil(async () => (await snapshot(page)).caches.length === 1 && (await snapshot(page)).caches[0] === 'cotizador-v7.8'); await page.waitForTimeout(700);
  const afterNow = await snapshot(page); assert.equal(navigations, navBeforeLater + 1); assert.equal(afterNow.localStorage.wilan_atomic_marker, 'preserve-me'); assert.deepEqual(afterNow.caches, ['cotizador-v7.8']); assert.equal(dialogs.length, 1); assert.equal(afterNow.canonicalFunction, true);
...
async function consistency(port) {
  ...
  await page.click('#pwa-update-now'); await waitUntil(async () => (await snapshot(page)).caches.includes('cotizador-v7.8')); await page.waitForLoadState('networkidle').catch(() => {}); await page.waitForTimeout(1200);
