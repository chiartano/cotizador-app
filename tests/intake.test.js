...
const quote = {
  ...
  receipt: { status: 'received', canOpenCrm: false, intakeOwnershipVersion: 'auth_uid.v1' },
};
const context = {
  console,
  setTimeout,
...
test('legacy receipt fails closed without calling Intake or inventing ownership', async () => {
  const compatibleReceipt = quote.receipt;
  quote.receipt = { status: 'received', canOpenCrm: false };
  const record = intake.prepare(quote, { kind: 'contact_call', date: '2026-09-01' });
  const result = await intake.send(record);
  assert.equal(result.ok, false);
  assert.equal(result.error.code, 'QUOTE_ACTION_LEGACY_NOT_SUPPORTED');
  assert.equal(sent.length, 0);
  assert.equal(intake.getByQuoteCommand(quote.commandId).status, 'legacy');
  quote.receipt = compatibleReceipt;
});
